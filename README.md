...
### Interactive Dip Gauge - a "wow" data-entry widget for Tank Measurement
...
The dialog is genuinely scaled to the real, selected Tank - not a fixed 0-100 scale: it reads that Oil Tank's actual `reference_height_mm`/`capacity_kl` and builds the ruler's tick marks and the tank's fill percentage from those real numbers (clicking the icon before a Tank is chosen gets a plain message asking to pick one first, rather than guessing a scale). A CSS-shaded cylinder (elliptical caps, gradient shading, an animated shimmer on the liquid surface) shows the tank filling in real time as you drag a handle up and down a ruler alongside it - built with a custom pointer-drag handler (not a native `<input type=range>`, whose vertical-orientation styling is inconsistent across browsers) so the drag behaviour and the visual are both fully controlled and stay in sync. The same dialog also has plain fields for temperature/free water dip/density, so "Apply to Form" fills the whole Dip Reading section in one action via real `frm.set_value()` calls - this is a data-entry convenience only; nothing is read from or written to the database until the form itself is saved normally. Observed Temperature and Free Water Dip default to real standard values rather than a bare `0.00` for the operator to type over every time: temperature defaults to the tank's own product's real `reference_temperature_c` (the same field VCF calculations already use - never a hardcoded constant across every product), falling back to 15C only when a product doesn't override it; Free Water Dip defaults to 0 (no free water), the correct assumption unless one is actually observed. Both stay fully editable - these are starting points, not fixed values.
