...
const LEDGER_ROWS = [
  {
    ts: '09:41:02',
    journey: 'JNY-00042',
    decision: 'AI Recommendation approved — Maintenance Work Order raised for Pump Station KP2',
  ...
  },
  {
    ts: '09:12:47',
    journey: 'JNY-00038',
    decision: 'Reconciliation variance 0.51% accepted with written justification',
...
export function DecisionLedger() {
  return (
    <section id='ledger' className='scroll-mt-24 mb-12'>
      <div className='mb-5'>
        <h2 className='text-foreground text-lg font-semibold tracking-tight'>Decision ledger</h2>
        <p className='text-muted-foreground mt-1 text-sm'>
          Append-only. Every AI-assisted decision and manual override, immutable the moment
          it&apos;s written.
        </p>
      </div>
      ...
      <Card>
        <CardContent className='px-0 pt-0'>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className='text-muted-foreground pl-6'>Timestamp</TableHead>
                <TableHead className='text-muted-foreground'>Journey</TableHead>
                <TableHead className='text-muted-foreground'>Decision</TableHead>
                <TableHead className='text-muted-foreground'>Decided by</TableHead>
                <TableHead className='text-muted-foreground pr-6'>Source</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {LEDGER_ROWS.map((row) => (
                <TableRow key={`${row.ts}-${row.journey}`}>
                  ...
                  <TableCell className='text-muted-foreground text-sm'>{row.decidedBy}</TableCell>
                  <TableCell className='pr-6'>{sourceBadge(row.source)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
